...
$result = $db->query("SELECT m.*, DATE_FORMAT(m.created_at, '%D-%b-%Y, %h:%i %p') AS time, CONCAT(u.firstname, ' ', u.lastname) AS fullname, u.designation, u.email FROM messages m LEFT JOIN users u ON m.to_user_id = u.id WHERE m.id = {$msg_id} ORDER BY m.created_at DESC")->first();
//var_dump($result);
